...
module Make (UserRepo : Repo.REPOSITORY) : Sihl.User.Sig.SERVICE = struct
  ...
  let get_by_token request token =
    ...
    in
    UserRepo.User.get ~id:token_user
  ...
  let get_all request user =
    if Sihl.User.is_admin user then
      UserRepo.User.get_all |> Sihl.Core.Db.query_db_exn request
    else
      Sihl.Core.Err.raise_no_permissions
        "user is not allowed to fetch all users"

  let get_by_email request ~email =
    UserRepo.User.get_by_email ~email |> Sihl.Core.Db.query_db_exn request
  ...
  let register ?(suppress_email = false) request ~email ~password ~username =
    ...
    if Result.is_ok user then
      ...
      in
      let* () =
        UserRepo.User.insert user |> Sihl.Core.Db.query_db_exn request
      in
      let* () =
  ...
  let logout request user =
    ...
    UserRepo.Token.delete_by_user ~id |> Sihl.Core.Db.query_db_exn request
  ...
  let token request user =
    let token = Sihl.User.Token.create user in
    let* result =
      UserRepo.Token.insert token |> Sihl.Core.Db.query_db request
    in
