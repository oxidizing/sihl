...
let xor_empty _ () =
  Alcotest.(
    check
      (option (list char))
      "XORs both empty" (Some []) (Sihl_web.Csrf.xor [] []));
  Lwt.return ()
  ...
  List.iter
    (fun (v, r) ->
      Alcotest.(
        check
          (option (list char))
          "Decrypts different length" None
          (Sihl_web.Csrf.decrypt_with_salt
...
let get_secret tk =
  ...
  |> Result.get_ok |> String.to_seq |> List.of_seq
  |> fun tk ->
  Sihl_web.Csrf.decrypt_with_salt ~salted_cipher:tk
    ~salt_length:(List.length tk / 2)
  |> Option.get |> List.to_seq |> String.of_seq
...
let two_get_requests_yield_same_token _ () =
  ...
  Alcotest.(
    check string "Same CSRF secret" (get_secret !token_ref1)
  ...
  Lwt.catch
    ...
    (function
      | Sihl_web.Csrf.Crypto_failed txt ->
          Alcotest.(
            check string "Raises" "Failed to decode CSRF token. Wrong padding"
              txt);
          Lwt.return ()
      | exn -> Lwt.fail exn)
...
let two_post_requests_succeed _ () =
  ...
  (* Do second POST *)
  Alcotest.(
    check bool "New token generated after POST" false
  ...
  [
    ( "encryption",
      [
        ...
        test_case "decrypt with salt length differs" `Quick
    ...
    ( "csrf",
      [
        ...
        test_case "two get requests yield same CSRF token" `Quick
        ...
        test_case "post request with invalid b64 CSRF token fails" `Quick
        ...
        test_case "post request with non-matching CSRF token fails" `Quick
          post_request_with_nonmatching_token_fails;
        test_case "post request with non-existing CSRF token fails" `Quick
        ...
        test_case "two post requests with a valid CSRF token succeed" `Quick
