let test_email_rendering_simple _ () =
  let data =
    Sihl.Email.Template.Data.empty |> Sihl.Email.Template.Data.add ~key:"foo" ~value:"bar"
  in
  let actual, _ =
    Sihl.Email.Template.render data (Sihl.Email.Template.make ~text:"{foo}" "test")
  in
  ...
  in
  let actual, _ =
    Sihl.Email.Template.render data (Sihl.Email.Template.make ~text:"{foo} {bar}" "test")
  in
