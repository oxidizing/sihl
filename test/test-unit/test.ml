...
let suite =
  [ ( "container"
    , [ test_case "order all dependencies" `Quick Test_container.order_all_dependencies
      ; test_case
  ...
  ; ( "email"
    , [ test_case "render email simple" `Quick Test_email.test_email_rendering_simple
      ; test_case "render email complex" `Quick Test_email.test_email_rendering_complex
      ] )
  ; ( "regex"
  ...
  ; ( "message"
    , [ test_case "entry to and from string" `Quick Test_message.entry_to_and_from_string
  ...
  ; ( "encryption"
    , [ test_case "xor results in empty" `Quick Test_encryption.xor_empty
      ; test_case "xor results in correct string" `Quick Test_encryption.xor_valid
      ; test_case "xor doesn't produce a result" `Quick Test_encryption.xor_length_differs
      ; test_case
          "decrypt results in empty"
          `Quick
          Test_encryption.decrypt_with_salt_empty
      ; test_case
          "decrypt results in correct string"
          `Quick
          Test_encryption.decrypt_with_salt_valid
      ; test_case
          "decrypt doesn't produce a result"
          `Quick
          Test_encryption.decrypt_with_salt_length_differs
      ] )
  ]
;;
