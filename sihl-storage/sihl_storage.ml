...
module Make (Repo : Repo.Sig) : Sihl_contract.Storage.Sig = struct
  ...
  let delete ~id =
    let* file = find ~id in
    let blob_id = Sihl.Storage.Stored.blob file in
  ...
  let upload_base64 ~file ~base64 =
    ...
    let stored_file = Sihl.Storage.Stored.make ~file ~blob:blob_id in
  ...
  let download_data_base64_opt ~file =
    ...
    | Some (Error (`Msg msg)) ->
      Logs.err (fun m ->
          m "STORAGE: Could not get base64 content of file %a" Sihl.Storage.Stored.pp file);
      raise (Sihl_contract.Storage.Exception msg)
    ...
    | None ->
      raise
        (Sihl_contract.Storage.Exception
           (Format.asprintf "File data not found for file %a" Sihl.Storage.Stored.pp file))
  ;;
