...
let decrypt_with_salt_empty _ () =
  Alcotest.(
    check
      ...
      "Decrypts empty"
      (Some [])
      (Sihl.Web.Csrf.decrypt_with_salt ~salted_cipher:[] ~salt_length:0));
  Lwt.return ()
;;
    ...
    (fun ((v1, v2), r) ->
      Alcotest.(
        check
          ...
          (Sihl.Web.Csrf.decrypt_with_salt
...
let apply_middlewares handler =
  handler
  |> Rock.Middleware.apply (Sihl.Web.Middleware.csrf ())
  |> Rock.Middleware.apply Sihl.Web.Middleware.form
  |> Rock.Middleware.apply (Sihl.Web.Middleware.session ())
;;
...
let get_request_yields_token _ () =
  ...
  let handler req =
    let token_value = Sihl.Web.Csrf.find req in
