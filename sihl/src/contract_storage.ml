...
module type Sig = sig
  ...
  val find_opt : ?ctx:(string * string) list -> id:string -> stored option Lwt.t

  val find : ?ctx:(string * string) list -> id:string -> stored Lwt.t
  val delete : ?ctx:(string * string) list -> id:string -> unit Lwt.t
