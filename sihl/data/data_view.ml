module PartialCollection = struct
  ...
  let controls_of_pagination page total_items =
    ...
    let first = if offset <= 0 then None else Some (Data_ql.Page.set_offset 0 page) in
    let previous =
      if offset - limit >= 0
      then Some (Data_ql.Page.set_offset (offset - limit) page)
      else None
    in
    let next =
    ...
    let last =
      ...
      then Some (Data_ql.Page.set_offset (total_items - 1) page)
      else None
    in
