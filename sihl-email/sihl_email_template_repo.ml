...
module MakeMariaDb (MigrationService : Sihl.Contract.Migration.Sig) : Sig = struct
  module Sql = struct
    ...
    let get ~id =
      Sihl.Service.Database.query (fun (module Connection : Caqti_lwt.CONNECTION) ->
          Connection.find_opt get_request id |> Lwt.map Sihl.Service.Database.raise_error)
    ;;
