...
let send_simple_email _ () =
  ...
  Alcotest.(check string "has recipient" "recipient@example.com" sent_email.recipient);
...
let send_inline_templated_email _ () =
  ...
  let* () = Sihl_facade.Email.send email in
  let sent_email = Sihl_facade.Email.inbox () |> List.hd in
  let html_rendered = "<html>hello walter, you have signed in 8 of times!</html>" in
  let text_rendered = "hello walter, you have signed in 8 of times!" in
  Alcotest.(check (option string) "has html" (Some html_rendered) sent_email.html);
...
let suite =
  [ ( "email"
    , [ test_case "create email template" `Quick create_template
      ...
      ; test_case "send inline templated email" `Quick send_inline_templated_email
