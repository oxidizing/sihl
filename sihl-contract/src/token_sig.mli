...
(** Create a token and store a token.
    ...
    Provide [expires_in] to define a duration in which the token is valid, default is one
    day. Provide [data] to store optional data as string. Provide [length] to define the
...
(** Invalidate a token by marking it as such in the database and therefore marking it "to
    be deleted" *)
