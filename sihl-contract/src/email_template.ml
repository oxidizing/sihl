...
(* Signature *)
let name = "sihl.service.email.template"

module type Sig = sig
  include Sihl_core.Container.Service.Sig

  val get : id:string -> t option Lwt.t
  val get_by_name : name:string -> t option Lwt.t
  val create : name:string -> html:string -> text:string -> t Lwt.t
  val update : template:t -> t Lwt.t
