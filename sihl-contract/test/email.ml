let test_email_rendering_simple _ () =
  let data =
    Sihl_contract.Email_template.Data.empty
    |> Sihl_contract.Email_template.Data.add ~key:"foo" ~value:"bar"
  in
  let actual, _ =
    Sihl_contract.Email_template.render
      data
      (Sihl_contract.Email_template.make ~text:"{foo}" "test")
  in
  Alcotest.(check string) "Renders template" "bar" actual;
  let data =
    Sihl_contract.Email_template.Data.empty
    |> Sihl_contract.Email_template.Data.add ~key:"foo" ~value:"hey"
    |> Sihl_contract.Email_template.Data.add ~key:"bar" ~value:"ho"
  in
  let actual, _ =
    Sihl_contract.Email_template.render
      data
      (Sihl_contract.Email_template.make ~text:"{foo} {bar}" "test")
  in
...
let test_email_rendering_complex _ () =
  ...
  let actual, _ =
    Sihl_contract.Email_template.render
      data
      (Sihl_contract.Email_template.make ~text:"{foo} {bar}{foo}" "test")
  in
