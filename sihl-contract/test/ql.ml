let query = Alcotest.testable Sihl_contract.Database.Ql.pp Sihl_contract.Database.Ql.equal

let to_string_limit_offset _ () =
  let query = Sihl_contract.Database.Ql.(empty |> set_offset 10 |> set_limit 20) in
  let actual = Sihl_contract.Database.Ql.to_string query in
  let expected = "((page((limit 20)(offset 10))))" in
  Lwt.return @@ Alcotest.(check string "equals" expected actual)
;;

let to_string_sort _ () =
  let query = Sihl_contract.Database.Ql.(empty |> set_sort [ Asc "foo"; Desc "bar" ]) in
  let actual = Sihl_contract.Database.Ql.to_string query in
...
let to_string_filter _ () =
  let criterion =
    Sihl_contract.Database.Ql.Filter.(C { key = "foo"; value = "bar"; op = Eq })
  in
  let query = Sihl_contract.Database.Ql.(empty |> set_filter criterion) in
  let actual = Sihl_contract.Database.Ql.to_string query in
...
let to_string _ () =
  let filter_criterion =
    Sihl_contract.Database.Ql.Filter.(C { key = "foo"; value = "bar"; op = Eq })
  in
  let sort_criterion = Sihl_contract.Database.Ql.Sort.[ Asc "foo"; Desc "bar" ] in
  let query =
    Sihl_contract.Database.Ql.(
      empty
...
let to_sql_limit_offset _ () =
  let query = Sihl_contract.Database.Ql.(empty |> set_offset 10 |> set_limit 20) in
  let actual, values = Sihl_contract.Database.Ql.to_sql [] query in
...
let to_sql_sort _ () =
  let query = Sihl_contract.Database.Ql.(empty |> set_sort [ Asc "foo"; Desc "bar" ]) in
  let actual, values = Sihl_contract.Database.Ql.to_sql [ "foo"; "bar" ] query in
...
let to_sql_filter _ () =
  ...
  let criterion1 =
    Sihl_contract.Database.Ql.Filter.{ key = "foo"; value = "bar"; op = Eq }
  in
  let criterion2 =
    Sihl_contract.Database.Ql.Filter.{ key = "fooz"; value = "baz"; op = Like }
  in
  let criterion3 =
    Sihl_contract.Database.Ql.Filter.{ key = "some"; value = "where"; op = Like }
  in
  let filter_criterions =
    Sihl_contract.Database.Ql.Filter.(
      Or [ And [ C criterion1; C criterion2 ]; C criterion3 ])
  in
  let query = Sihl_contract.Database.Ql.(empty |> set_filter filter_criterions) in
  let actual, values = Sihl_contract.Database.Ql.to_sql whitelist query in
...
let to_sql _ () =
  ...
  let query, values = Sihl_contract.Database.Ql.to_sql whitelist query in
  let expected_query =
...
let to_sql_fragments _ () =
  ...
  let filter, sort, pagination, values =
    Sihl_contract.Database.Ql.to_sql_fragments filter_whitelist query
  in
...
let of_string_empty_sort _ () =
  let actual = Sihl_contract.Database.Ql.of_string "" in
  Lwt.return
  @@ Alcotest.(
       check (result query string) "equals" (Ok Sihl_contract.Database.Ql.empty) actual)
;;

let of_string_sort _ () =
  let actual =
    Sihl_contract.Database.Ql.of_string "((sort((Asc foo)(Desc bar)))(page()))"
  in
  let expected =
    Sihl_contract.Database.Ql.(empty |> set_sort [ Asc "foo"; Desc "bar" ])
  in
  Lwt.return @@ Alcotest.(check (result query string) "equals" (Ok expected) actual)
;;

let of_string_limit_offset _ () =
  let actual = Sihl_contract.Database.Ql.of_string "((page((limit 20)(offset 10))))" in
  let expected = Sihl_contract.Database.Ql.(empty |> set_limit 20 |> set_offset 10) in
  Lwt.return @@ Alcotest.(check (result query string) "equals" (Ok expected) actual)
;;

let of_string_filter _ () =
  let actual =
    Sihl_contract.Database.Ql.of_string
      "((filter(C((key foo)(value bar)(op Eq))))(page()))"
  in
  let criterion =
    Sihl_contract.Database.Ql.Filter.(C { key = "foo"; value = "bar"; op = Eq })
  in
  let expected = Sihl_contract.Database.Ql.(empty |> set_filter criterion) in
  Lwt.return @@ Alcotest.(check (result query string) "equals" (Ok expected) actual)
;;
