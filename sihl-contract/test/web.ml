...
let externalize_link _ () =
  let actual = Sihl_contract.Http.externalize_path ~prefix:"prefix" "foo/bar" in
...
let prefix_route _ () =
  let route =
    Sihl_contract.Http.get "/users" (fun _ ->
        Lwt.return (Opium.Response.of_plain_text ""))
  in
  let _, prefixed_path, _ = Sihl_contract.Http.prefix "/admin" route in
...
let router_middleware _ () =
  ...
  in
  let route2 =
    Sihl_contract.Http.get "/users/:id" (fun _ ->
  ...
  in
  let _, _, handler = Sihl_contract.Http.router_to_routes router |> List.hd in
