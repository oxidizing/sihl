let should_run_job _ () =
  ...
  in
  let job_instance =
    Sihl_contract.Queue_job_instance.create ~input:None ~delay:None ~now job
  in
  let actual = Sihl_contract.Queue_job_instance.should_run ~job_instance ~now in
  Alcotest.(check bool) "pending job should run" true actual;
  let delay = Some Sihl_core.Time.OneDay in
  let job_instance =
    Sihl_contract.Queue_job_instance.create ~input:None ~delay ~now job
  in
  let actual = Sihl_contract.Queue_job_instance.should_run ~job_instance ~now in
  Alcotest.(check bool)
  ...
  let job_instance =
    Sihl_contract.Queue_job_instance.create ~input:None ~delay:None ~now job
    |> Sihl_contract.Queue_job_instance.incr_tries
    |> Sihl_contract.Queue_job_instance.incr_tries
  in
  let actual = Sihl_contract.Queue_job_instance.should_run ~job_instance ~now in
  ...
  let job_instance =
    Sihl_contract.Queue_job_instance.create ~input:None ~delay:None ~now job
    |> Sihl_contract.Queue_job_instance.set_failed
  in
  let actual = Sihl_contract.Queue_job_instance.should_run ~job_instance ~now in
  Alcotest.(check bool) "failed job should not run" false actual;
  let job_instance =
    Sihl_contract.Queue_job_instance.create ~input:None ~delay:None ~now job
    |> Sihl_contract.Queue_job_instance.set_succeeded
  in
  let actual = Sihl_contract.Queue_job_instance.should_run ~job_instance ~now in
  Alcotest.(check bool) "succeeded job should not run" false actual;
  let workable_job = job |> Sihl_contract.Queue_workable_job.of_job in
  let job_instance =
    Sihl_contract.Queue_job_instance.create ~input:None ~delay:None ~now job
    |> Sihl_contract.Queue_job_instance.update_next_run_at workable_job
  in
  let actual = Sihl_contract.Queue_job_instance.should_run ~job_instance ~now in
