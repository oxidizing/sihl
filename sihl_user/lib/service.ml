...
module User = struct
  let is_valid_auth_token request token =
    ...
    let* token =
      Repository.Token.get ~value:token |> Sihl.Db.query_db request
    in
    token |> Result.ok
  ...
  let get request user ~user_id =
    ...
    if Model.User.is_admin user || Model.User.is_owner user user_id then
      let* user = Repository.User.get ~id:user_id |> Sihl.Db.query_db request in
      user
      |> Sihl.Err.with_bad_request ("could not find user with id " ^ user_id)
      |> Lwt.return
    else Sihl.Err.raise_no_permissions "user is not allowed to fetch user"
  ...
  let get_by_token request token =
    ...
    in
    let token_user =
      token |> Sihl.Err.with_not_authenticated |> Model.Token.user
    in
    Repository.User.get ~id:token_user
    |> Sihl.Db.query_db request
    |> Lwt.map Sihl.Err.with_not_authenticated
  ...
  let get_all request user =
    ...
    if Model.User.is_admin user then
      Repository.User.get_all |> Sihl.Db.query_db_exn request
    else Sihl.Err.raise_no_permissions "user is not allowed to fetch all users"

  let get_by_email request ~email =
    let (module Repository : Contract.REPOSITORY) =
      Sihl.Registry.get Binding.Repository.key
    in
    Repository.User.get_by_email ~email |> Sihl.Db.query_db_exn request
  ...
  let register ?(suppress_email = false) request ~email ~password ~username =
    ...
    let* user =
      Repository.User.get_by_email ~email |> Sihl.Db.query_db request
    in
    if Result.is_ok user then Sihl.Err.raise_bad_request "email already taken"
    else
      let user =
        Model.User.create ~email ~password ~username ~admin:false
          ~confirmed:false
      in
      let* () = Repository.User.insert user |> Sihl.Db.query_db_exn request in
      let* () =
  ...
  let logout request user =
    ...
    Repository.Token.delete_by_user ~id |> Sihl.Db.query_db_exn request
  ...
  let login request ~email ~password =
    ...
    if Model.User.matches_password password user then
      ...
      let* () = Repository.Token.insert token |> Sihl.Db.query_db_exn request in
      Lwt.return token
    else Sihl.Err.raise_not_authenticated "wrong credentials provided"
  ...
  let token request user =
    ...
    let token = Model.Token.create user in
    let* result = Repository.Token.insert token |> Sihl.Db.query_db request in
    let () = result |> Sihl.Err.with_database "failed to store token" in
    Lwt.return token
  ...
  let update_password request current_user ~email ~old_password ~new_password =
    ...
    then
      ...
      let* () =
        Repository.User.update updated_user |> Sihl.Db.query_db_exn request
      in
      Lwt.return updated_user
    else Sihl.Err.raise_no_permissions "user is not allowed to update this user"
  ...
  let set_password request current_user ~user_id ~password =
    ...
    let* user = Repository.User.get ~id:user_id |> Sihl.Db.query_db request in
    let user =
      user |> Sihl.Err.with_bad_request "user to set password not found"
    in
    let _ =
    ...
    if Model.User.is_admin current_user then
      ...
      in
      Lwt.return updated_user
    else Sihl.Err.raise_no_permissions "user is not allowed to set password"
  ...
  let confirm_email request token =
    ...
    let* token =
      Repository.Token.get ~value:token |> Sihl.Db.query_db request
    in
    let token = token |> Sihl.Err.with_bad_request "invalid token provided" in
    if not @@ Model.Token.is_valid_email_configuration token then
      Sihl.Err.raise_bad_request "invalid confirmation token provided"
    else
      Sihl.Db.query_db_with_trx_exn request (fun connection ->
          let* () =
            Repository.Token.update (Model.Token.inactivate token) connection
            |> Sihl.Err.database
          in
          let* user =
            Repository.User.get ~id:token.user connection |> Sihl.Err.database
          in
  ...
  let reset_password request ~token ~new_password =
    ...
    if not @@ Model.Token.can_reset_password token then
      Sihl.Err.raise_bad_request "invalid or inactive token provided"
    else
      let* user =
        Repository.User.get ~id:token.user |> Sihl.Db.query_db request
      in
      let user =
        user |> Sihl.Err.with_bad_request "invalid user for token found"
      in
      ...
      Repository.Token.update token |> Sihl.Db.query_db_exn request
end
