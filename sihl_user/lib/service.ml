...
module User = struct
  ...
  let get request user ~user_id =
    ...
    if Model.User.is_admin user || Model.User.is_owner user user_id then
      ...
      in
      user
      |> Sihl_core.Err.with_bad_request
           ("could not find user with id " ^ user_id)
      |> Lwt.return
    else Sihl_core.Err.raise_no_permissions "user is not allowed to fetch user"
  ...
  let get_by_token request token =
    ...
    in
    let token_user =
      token |> Sihl_core.Err.with_not_authenticated |> Model.Token.user
    in
    Repository.User.get ~id:token_user
    |> Sihl_core.Db.query_db request
    |> Lwt.map Sihl_core.Err.with_not_authenticated
  ...
  let get_all request user =
    ...
    if Model.User.is_admin user then
      ...
      Sihl_core.Err.raise_no_permissions
  ...
  let send_registration_email request user =
    ...
    result |> Sihl_core.Err.with_email |> Lwt.return
  ...
  let register ?(suppress_email = false) request ~email ~password ~username =
    ...
    if Result.is_ok user then
      Sihl_core.Err.raise_bad_request "email already taken"
    else
      let user =
  ...
  let login request ~email ~password =
    ...
    if Model.User.matches_password password user then
      ...
      in
      Lwt.return token
    else Sihl_core.Err.raise_not_authenticated "wrong credentials provided"
  ...
  let token request user =
    ...
    in
    let () = result |> Sihl_core.Err.with_database "failed to store token" in
    Lwt.return token
  ...
  let set_password request current_user ~user_id ~password =
    ...
    in
    let user =
      user |> Sihl_core.Err.with_bad_request "user to set password not found"
    in
    let _ =
    ...
    if Model.User.is_admin current_user then
      ...
      Sihl_core.Err.raise_no_permissions "user is not allowed to set password"
  ...
  let reset_password request ~token ~new_password =
    ...
    if not @@ Model.Token.can_reset_password token then
      Sihl_core.Err.raise_bad_request "invalid or inactive token provided"
    else
      let* user =
        Repository.User.get ~id:token.user |> Sihl_core.Db.query_db request
      in
      let user =
        user |> Sihl_core.Err.with_bad_request "invalid user for token found"
      in
