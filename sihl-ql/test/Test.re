  ...
  describe("End to end", () => {
    ...
    describe("ordering", () => {
      it("order=age.desc,height.asc", () => {
        ...
        let expected = Belt.Result.Ok("ORDER BY age DESC height ASC");
        Assert.deep_equal(actual, expected);
      });
      it("order=age", () => {
        let actual = Parser.parse("order=age") <$> Sql.generate;
        let expected = Belt.Result.Ok("ORDER BY age ASC");
        Assert.deep_equal(actual, expected);
      });
    });
    it("pagination", () => {
      it("limit=15&offset=30", () => {
        ...
        let expected = Belt.Result.Ok("LIMIT 15 OFFSET 30");
        Assert.deep_equal(actual, expected);
      })
    });
  });
