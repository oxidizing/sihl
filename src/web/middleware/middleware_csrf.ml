...
struct
  ...
  let create_secret ctx =
    let* token =
      TokenService.create ctx ~kind:"csrf" ~length:csrf_token_length ()
    in
  ...
  let m () =
    let filter handler ctx =
      ...
      if Http.Req.is_get ctx then handler ctx
      else
