  ...
  let filter handler ctx =
    ...
    if Web_req.is_get ctx then
      ...
      if local_path |> String.is_prefix ~prefix:uri_prefix then
        let request_if_none_match = Web_req.get_header ctx "If-None-Match" in
        public_serve local_map ~requested:local_path ~request_if_none_match
          ?etag_of_fname ?headers ()
        >>= function
        | `Not_found -> handler ctx
        | `Ok x -> return Web_res.(html |> set_opium_res x)
      else handler ctx
    else handler ctx
  in
  Web_middleware_core.create ~name:"static" filter
