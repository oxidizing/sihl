...
type t = { data : data; schema : schema }
  ...
  let schema =
    ...
    | Some schema ->
        Conformist.fold_left
          ~f:(fun schema field -> List.cons field schema)
          ~init:[] schema
    | None -> []
  in
  { data; schema }
...
let read schema =
  ...
  | errors ->
      let errors =
        List.map
          (fun (k, v) -> Format.sprintf "Key '%s' as invalid value: %s" k v)
          errors
      in
...
let is_testing () =
  match read_string "SIHL_ENV" with Some "test" -> true | _ -> false
