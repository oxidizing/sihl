...
(* TODO [jerben]
  0. store ref to current app and start ctx
  1. loop forever (in Lwt_main.run)
  2. when command finishes, exit loop
  3. when SIGINT comes, exit loop
  4. call stop app
  let stop app ctx =
    let* () = app.before_stop ctx in
    print_endline "CORE: Stop services";
    let* () = Container.stop_services ctx app.services in
    print_endline "CORE: Services stopped";
    app.after_stop ctx *)

let starting_commands service =
  (* When executing a starting command, the service that publishes that command and all its dependencies is started before the command is run *)
  List.map
    (fun command ->
      let fn args =
...
let run ?(commands = []) ?(configuration = [])
    ?(log_reporter = Log.default_reporter) app =
  ...
  let configurations =
    List.map
      (fun service -> Container.Service.configuration service)
      app.services
  in
  List.iter
    (fun configuration ->
      configuration |> Configuration.data |> Configuration.store)
    configurations;
  ...
  in
  let commands =
    List.concat [ configuration_commands; service_commands; commands ]
  in
